...
//
// Per-pixel null-geodesic ray tracer. Integrates each camera ray through the
// Schwarzschild or Kerr spacetime and resolves it against the accretion
// disc, event horizon, and a procedural starfield. The maths here is the
// exact equivalent of the CPU `cpu-render` / `kerr-cpu-render` paths in
// `cli/main.cpp` — the same shared_shader/*.h headers are included, so any
// bug fixed in those headers flows to every backend.
//
// Entry point: `geodesic_kernel` compute kernel. One thread per pixel,
// 8×8 threadgroups (Apple GPU sweet spot for register pressure vs.
// occupancy at this compute depth).
...
// Tanner-Helland-style blackbody → sRGB approximation. Cheap polynomial fit
// that matches the CPU `blackbody_srgb_tanner_helland` to ~3% across the disc
// temperature range (1000K–30000K).
static inline float3 blackbody_srgb(float T) {
    T = clamp(T, 1000.0f, 40000.0f) * 0.01f;   // work in units of 100K
    float r, g, b;
    if (T <= 66.0f) {
        r = 1.0f;
...
// Novikov–Thorne temperature profile: T(r) ∝ (r/r_inner)^(-3/4) ·
// (1 − √(r_inner/r)). The raw profile peaks at u = √(r_inner/r) = 0.6
// (r ≈ 2.78 r_inner) with value ≈ 0.186. We divide by that max so the
// `peak_T` argument is the *actual peak temperature* the disc reaches —
// a stellar-BH-sensible 15 000 K peak_T then produces 15 000 K inside
// the hot ring, not 3 000 K which the raw formula would give.
static inline float disc_temperature(float r_cross, float r_inner, float peak_T) {
    ...
    // 0.186 = (0.6)^1.5 · 0.4 (profile peak; see comment above).
    return peak_T * raw / 0.186f;
}

// Procedural spiraling-band accretion texture. The disc otherwise reads as
// a featureless gradient; real accretion flows show filamentary structure
// that winds around the BH because matter at smaller r orbits faster
// (Keplerian dΩ/dr < 0). Two layers of trigonometric noise, one fine in
// radius and one coarser, combined multiplicatively with phi shear.
static inline float disc_band_mask(float r_cross, float phi_cross,
                                   float r_inner, float turbulence,
                                   float time_sec) {
    // Logarithmic radial coord emphasises the inner disc where flow is
    // compressed relative to the outer regions.
    const float rho = log(max(r_cross / r_inner, 0.1f));
    // Keplerian phase shear: phi_eff = phi - Ω(r) t_λ ≈ phi - √(M/r³) factor.
    const float shear = 4.0f / max(pow(r_cross / r_inner, 1.5f), 0.25f);
    // Animation: rotate the band pattern in +phi at the local Keplerian
    // angular velocity ω(r) ∝ r^(-3/2). Inner bands sweep visibly; outer
    // bands move slowly. The 0.8 prefactor is tuned so the inner edge
    // makes a full revolution in roughly 8 seconds — fast enough to
    // read as motion, slow enough to not strobe under a 60 Hz display.
    ...
    const float combined = mix(0.8f, band1 * band1 * band2, turbulence);
    // Keep a 65% baseline so the disc never goes completely dark between
    // bands — it would look like a striped beachball otherwise. Bright
    // peaks can exceed 1.0 to feed the bloom extract.
    return clamp(0.65f + 0.55f * combined, 0.0f, 1.3f);
}

// Procedural starfield — sparse bright pinpoints on a near-black sky. The
// previous version layered a Milky-Way-style latitude tint on top of every
// pixel, which read as "off-white gray" instead of black space once bloom
// + ACES lifted the highlights. The current pass: pure dark-blue base
// (≈ RGB 0/0.4/1.5 % linear), 1.5 % cell density, sharp brightness
// power-law so the visible stars *pop* against true black.
static inline uint xorshift32(uint x) {
    x ^= x << 13; x ^= x >> 17; x ^= x << 5;
    return x;
}
static inline uint mix32(uint a, uint b) {
    return xorshift32(a * 0x9E3779B9u + b);
}

static inline float3 starfield_color(float3 dir) {
    ...
    // Near-black base with a faint blue tint. No latitude gradient — real
    // deep space *is* mostly black; the Milky Way would belong on its own
    // texture layer if we want it back later.
    float r = 0.0015f;
    float g = 0.002f;
    float b = 0.005f;
    ...
    if (has_star) {
        ...
        // Steeper power-law than before — fewer dim stars, sharper peaks.
    ...
    // Scale the starfield down so the HDR disc (with g⁴ peaks in the
    // 10–100 range) still dominates the tone-mapped frame. 0.6 keeps the
    // popped stars visible while leaving headroom for the bloom pass.
    return float3(r, g, b) * 0.6f;
}

// Pack Cartesian ray direction into a (u^r, u^θ, u^φ) coordinate 4-velocity
// in the local Boyer-Lindquist basis at the camera. Same calculation the
// Schwarzschild and Kerr CPU paths share.
struct CameraRay {
    float3 cart;          // unit direction in Cartesian, for starfield sampling
    float  ur;            // coordinate u^r
    float  utheta;        // coordinate u^θ
    float  uphi;          // coordinate u^φ
    float  cam_r;
    float  cam_theta;
    float  cam_phi;
    float  sin_th;
...
static inline CameraRay build_camera_ray_subpixel(
    uint2 gid, float2 sub_offset, constant Uniforms& u)
{
...
// ----------------------------------------------------------------------------
// Schwarzschild ray integration. 1:1 with run_cpu_render in cli/main.cpp.
// ----------------------------------------------------------------------------

static inline float3 trace_schwarzschild(CameraRay cam, constant Uniforms& u) {
    State s;
    s.t     = 0.0f;
    s.r     = cam.cam_r;
    ...
    for (uint step = 0; step < u.max_steps; ++step) {
        if (!isfinite(s.r) || s.r < u.horizon_cut) return float3(0.0f);
        if (s.r > u.escape_r) {
            ...
            if (in_disc_annulus(r_cross, u.disc_r_inner, u.disc_r_outer)) {
                ...
                if (T_local > 0.0f) {
                    ...
                    if (u.flags & SING_FLAG_DOPPLER_ON) {
                        const float omega = sqrt(u.mass_M / (r_cross * r_cross * r_cross));
                    ...
                    const float flux = clamp(pow(u.disc_r_inner / r_cross, 3.0f),
                                             0.05f, 1.5f);
                    // Interpolate phi at the disc crossing so the band
                    // pattern is continuous across neighbouring rays.
                    const float phi_cross = lerp_scalar(/*prev=*/s.phi,
                                                        /*curr=*/s.phi, xing.frac);
                    const float bands = disc_band_mask(r_cross, phi_cross,
                                                       u.disc_r_inner,
                                                       u.disc_turbulence,
                                                       u.time_sec);
                    // True HDR output: I_obs = g⁴ I_emit (Rybicki & Lightman
                    // §4.9). The tone-mapping pass downstream compresses
                    // the dynamic range, and the bloom pass extracts the
                    // overbright pixels for glare.
                    const float g2 = g * g;
...
// ----------------------------------------------------------------------------
// Kerr ray integration via the canonical-momenta Hamiltonian form.
// 1:1 with run_kerr_cpu_render in cli/main.cpp (sans CPU supersampling).
// ----------------------------------------------------------------------------

static inline float3 trace_kerr(CameraRay cam, constant Uniforms& u) {
    ...
    // Metric components at camera — solve null condition g_μν u^μ u^ν = 0
    // for u^t > 0 (future-directed). See run_kerr_cpu_render §970–998.
    ...
    if (disc_q < 0.0f) return float3(0.0f);
    ...
    // Larger bail radius for Kerr since frame-dragging can extend integration.
    const float horizon_cut_kerr = 1.02f
        * (M + sqrt(max(M * M - a * a, 0.0f)));   // r+ with a small margin

    for (uint step = 0; step < u.max_steps; ++step) {
        if (!isfinite(s.r) || s.r < horizon_cut_kerr) return float3(0.0f);
        if (s.r > u.escape_r) {
            if (u.flags & SING_FLAG_STARFIELD_ON) {
                const float sx = sin(s.theta) * cos(s.phi);
                const float sy = sin(s.theta) * sin(s.phi);
        ...
        if ((u.flags & SING_FLAG_DISC_ON) && xing.crossed) {
            ...
            if (in_disc_annulus(r_cross, u.disc_r_inner, u.disc_r_outer)) {
                const float T_local = disc_temperature(r_cross, u.disc_r_inner,
                                                       u.disc_peak_T);
                if (T_local > 0.0f) {
                    // Gravitational redshift (Schwarzschild approx — Kerr-exact
                    // is one step more but the visual delta is small for a/M ≤ 0.9).
                    ...
                    float g_dop = 1.0f;
                    if (u.flags & SING_FLAG_DOPPLER_ON) {
                        // Kerr-modified Keplerian angular velocity — prograde.
    ...
    uint2                           gid    [[thread_position_in_grid]])
{
    ...
    for (uint sy = 0; sy < ss; ++sy) {
        for (uint sx = 0; sx < ss; ++sx) {
            const float2 off = float2((float(sx) + 0.5f) * sub_w,
                                      (float(sy) + 0.5f) * sub_w);
            const CameraRay cam = build_camera_ray_subpixel(gid, off, u);
            const float3 c = (u.metric_type == SING_METRIC_KERR)
                             ? trace_kerr(cam, u)
    ...
    output.write(float4(acc, 1.0f), gid);
}
